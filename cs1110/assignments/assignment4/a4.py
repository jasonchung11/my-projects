...
def draw_hex(t, s):
    ...
    t.color = color_org
    # color is unchanged due to draw_triangle() code
    #
...
def draw_spiral(w, side, ang, n, sp):
    ...
    + 'iterations',side)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert is_number(ang), report_error('ang is not a valud angle', ang)
...
def multi_polygons(w, side, k, n, sp):
    ...
    +'length',side)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert is_valid_iteration(k), report_error('k is not a valid of ' \
    'polygons', k)
    assert (type(n) == int and n >= 3), report_error('n is not a valid ' \
    + 'number of sides',n)
...
def multi_polygons_helper(t, side, k, n, sp):
    ...
    + 'length',side)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert is_valid_iteration(k), report_error('k is not a valid number of ' \
    + 'polygons',k)
    assert (type(n) == int and n >= 3), report_error('n is not a valid ' \
    +'number of sides',n)
...
def draw_polygon(t, side, n):
    ...
    assert is_valid_length(side), report_error('side is not a valid ' \
    + 'length',side)
    assert type(n) == int and n >= 1, report_error('n is an invalid # ' \
    + 'of poly sides',n)
...
def radiate(w, side, n, sp):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(side), report_error('side is not a ' \
    + 'valid length',side)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert (type(n) == int and n >= 2), report_error('n is not a valid ' \
    + 'number of lines',n)
...
def radiate_helper(t, side, n, sp):
    ...
    assert is_valid_length(side), report_error('side is not a valid ' \
    + 'length',side)
    assert (type(n) == int and n >= 2), report_error('n is an invalid ' \
    +'# of petals',n)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    ...
    for x in range(0,n):
        ...
        if t.heading >= 360.0:
            t.heading = t.heading-360.0
        elif t.heading < 0:
            t.heading = 360+t.heading

        hsl = introcs.HSL(t.heading,1.0,0.65)
...
def triangle(w, side, d, sp):
    ...
    assert is_valid_length(side), report_error('side is not ' \
    + 'a valid length', side)
    assert is_valid_depth(d), report('d is not a valid recursive ' \
    'depth for the triangle',d)
    assert is_valid_speed(sp), report('sp is not a valid speed', sp)
    # HINT: w.clear() clears window.
    # HINT: Set the visible attribute to False at the end, and remember to flush
    w.clear()
    p = Pen(w)
    ...
    triangle_helper(p,x,y,color,side,d)
    p.flush()
    p.visible = False
...
def triangle_helper(p, x, y, color, side, d):
    ...
    assert (type(color) == introcs.RGB), report_error('color is not ' \
    + 'a valid RGB color',color)
    assert is_valid_length(side), report_error('side is not a ' \
    + 'valid length', side)
    assert is_valid_depth(d), report_error('d is not a valid recursion depth',d)
    ...
    else:    # Recursive Case d > 0
        p.fillcolor = introcs.RGB(255-color.red, 255-color.green, 255-color.blue)
        fill_triangle(p,x,y-side*math.sqrt(3)/8,side/2,False)
        p.fillcolor = color
        triangle_helper(p,x-side/4,y-side*math.sqrt(3)/8,color,side/2,d-1)
        triangle_helper(p,x,y+side*math.sqrt(3)/8,color,side/2,d-1)
        triangle_helper(p,x+side/4,y-side*math.sqrt(3)/8,color,side/2,d-1)
...
def arrowhead(w, side, d, sp):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(side), report_error('side is not a valid side ' \
    + 'length',side)
    assert is_valid_depth(d), report_error('d is not a valid recursion depth',d)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    ...
    t.color = 'sea green'
    t.speed = sp
    t.move(-side/2,-side*math.sqrt(3)/8)
    t.heading = 0
...
def arrowhead_helper(t, length, left, d):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_valid_turtlemode(t), report_error('Invalid turtle mode', t)
    assert is_valid_length(length), report_error('length is not a valid side ' \
    +'length',length)
    assert (type(left) == bool), report_error('left is not a valid ' \
    + 'orientation',left)
    assert is_valid_depth(d), report_error('d is not a valid recursion depth',d)
    ...
    if d == 0:
        ...
        if left == True:
            t.left(60)
            arrowhead_helper(t,length/2,False,d-1)
            t.right(60)
            arrowhead_helper(t,length/2,True,d-1)
            t.right(60)
            arrowhead_helper(t,length/2,False,d-1)
            t.left(60)
