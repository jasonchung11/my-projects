 ...
 * This assignment was more challenging than previous ones, but the recursion
 * examples provided in the videos helped a lot. It was helpful in understanding
 * the applications of trees through a real-world exapmle, and we thought the
 * visual was very cool as well.
 */
...
public class CovidTree {
    ...
    public static double timeSpent= 3.5;
    ...
    public CovidTree insert(Human p, Human c) throws IllegalArgumentException {
        ...
        CovidTree n= new CovidTree(c);
        node(p).children.add(n);
        return n;
//        CovidTree b= new CovidTree(c);
//        a.children.add(b);
//        return a;
    }
    ...
    public List<Human> CovidRouteTo(Human c) {
        ...
        // Searching Method
        if (human().equals(c)) {
            var base= new LinkedList<Human>();
            base.addFirst(human());
            return base;
        }

        for (CovidTree dt : children) {
            List<Human> out= dt.CovidRouteTo(c);
            if (out != null) {
                ((LinkedList<Human>) out).addFirst(human());
                return out;
    ...
    public Human commonAncestor(Human child1, Human child2) {
        ...
        Object[] a= CovidRouteTo(child1).toArray();
        Object[] b= CovidRouteTo(child2).toArray();
        int n= Math.min(a.length, b.length);
        for (int i= 0; i < n; i++ ) {
            if (a[i] != b[i]) return (Human) a[i - 1];
        }
        return (Human) a[n - 1];
        // is contains() too much time?
        // 10 lines
    }
    ...
    public boolean equals(Object ob) {
        ...
        CovidTree obj= (CovidTree) ob;
        if (human() != obj.human()) return false;
        if (size() == obj.size()) {
            for (CovidTree dt : obj.children) {
                if (!isInSet(dt, children)) return false;
            }
