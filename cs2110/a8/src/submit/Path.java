...
public class Path {
    ...
    public static List<Node> findShortestPathToGold(FleeState state) {
        Node v= state.currentNode();
        ...
        while (F.size() != 0) {
            ...
            var f= F.poll();
            if (f.getTile().gold() > 0) { return pathToEnd(SandF, f); }
            int distvf= SandF.get(f).dist;

            for (Edge e : f.exits()) {

                Node w= e.other(f);
                int newd= e.length + distvf;
                Info oldwInfo= SandF.get(w);
                Info newwInfo= new Info(newd, f);

                if (oldwInfo == null) {
                    F.insert(w, newd);
                    SandF.put(w, newwInfo);
                } else if (newd < oldwInfo.dist) {
                    SandF.put(w, newwInfo);
                    F.changePriority(w, newd);
                }
            }
