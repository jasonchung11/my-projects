...
/** An instance is a turtle in a panel within a JFrame. <br>
 * A turtle is a spot on the panel that you can move from place to place. <br>
...
public class Turtle {
	...
	/** The turtle points in direction angle: 0 <= angle < 360.<br>
	...
	/** Constructor: a black turtle starting at the middle of the<br>
	 * panel with angle 0 (looking east).<br>
	 * East (right) is angle 0; north (up), 90; west (left), 180;<br>
	 * South (down). 270. The pen is down. */
	public Turtle() {
		this(width / 2, height / 2, 0);
	}

	/** Constructor: a black turtle starting at (x,y) with angle ang.<br>
	 * East (right) is angle 0; north (up), 90; west (left), 180;<br>
	 * South (down), 270. The pen is down. */
	...
	/** = the Color value for c. <br>
	 * The values are as follows: 1 black, 2 blue, 3 cyan, 4 dark gray, <br>
	 * 5 gray, 6 green, 7 light gray, 8 magenta, 9 orange, 10 pink, 11 red, <br>
	 * 12 white, 13 yellow, any other integer black. */
	...
	/** = angle of the turtle (in degrees). <br>
	 * East (right) is angle 0; north (up), 90; west (left), 180; South (down), 270. */
	...
	/** Set the angle to ang degrees. <br>
	 * East (right) is angle 0; north (up), 90; west (left), 180; South (down), 270. */
	...
	/** Fill polygon given by points (x[i], y[i]) for i in 0..pts-1. */
	public void fillPolygon(int[] x, int[] y, int pts) {
		graphics.fillPolygon(x, y, pts);
	}

	/** Draw an equilateral triangle with top corner <br>
	 * at current pen position with side length s. */
	public void drawTriangle(double s) {
		double saveX= x;
		double saveY= y;
		double saveAngle= angle;
		angle= -60;
		for (int i= 0; i < 3; i+= 1) {
			move(s);
			addAngle(-120);
	...
	static double cos60degrees= .5;

	/** Draw an equilateral triangle with top point <br>
	 * at (x, y) and side length s. */
	public void drawTriangle(double x, double y, double s) {
		double lbx= x - s / 2;
		double lby= y + s * sin60degrees;
		double rbx= x + s / 2;
		double rby= y + s * sin60degrees;
		drawLine(x, y, lbx, lby);
		drawLine(x, y, rbx, rby);
		drawLine(lbx, lby, rbx, rby);
//		double saveX= x;
//		double saveY= y;
//		double saveAngle= angle;
//		angle= -60;
...
//		angle= saveAngle;
	...
	public void drawLine(double x1, double y1, double x2, double y2) {
		graphics.drawLine((int) Math.round(x1), (int) Math.round(y1),
	...
	/** Move the turtle d units in its current direction. <br>
	 * If the pen is down, a line will be drawn; <br>
	 * if the pen is up, it won't be drawn. */
